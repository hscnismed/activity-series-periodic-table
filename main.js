...
function getDropzoneFromTouch(touch) {
  ...
  while (el && !el.classList.contains("dropzone")) {
    el = el.parentElement;
  }
  return el;
}
...
metals.forEach((symbol) => {
  ...
  div.addEventListener("touchend", (e) => {
    ...
    if (dropTarget) {
      const targetSymbol = dropTarget.textContent.replace("✓ ", "").trim();
    ...
    if (matchedElement) {
      ...
      if (metals.includes(symbol)) {
        ...
        cell.addEventListener("drop", (e) => {
          ...
          if (droppedSymbol === symbol) {
            cell.style.backgroundColor = metalColors[droppedSymbol];
